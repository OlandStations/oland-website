 ...
 *
 * TODO (before launch): the 7 custom contact properties below must
 * exist in HubSpot with these EXACT internal names and be added to the
 * form, otherwise the submission is rejected:
...
const OPTION_CHOICES = [
  { key: "A", label: "Rent", value: "Rent" },
  { key: "B", label: "Purchase", value: "Purchase" },
  { key: "C", label: "Both", value: "Both" },
];
const POTABLE_CHOICES = [
  { key: "A", label: "Yes", value: "Yes" },
  { key: "B", label: "No", value: "No" },
  { key: "C", label: "Not sure", value: "Not sure" },
];
  ...
  const submit = useCallback(
    async (payloadData: FormData) => {
      ...
      try {
        ...
        // /api/create-deal before creating the rental item.
        (async () => {
          const dealPayload = {
...
function StepBody({
  ...
  switch (step) {
      ...
      return (
        <div>
          ...
          <p className="mt-5 text-lg text-white/85">
            A few quick questions about your event and we&rsquo;ll be in touch shortly. It takes
            about a minute.
          </p>
          <button
            type="button"
          ...
          >
            Start
            <span className="text-sm font-medium normal-case opacity-80">press Enter ↵</span>
          </button>
        </div>
      );
    ...
    case STEP.CONTACT:
      return (
        <Question label="First, who are we talking to?">
          <div className="space-y-4">
            ...
            <TextField
              ...
              hint={data.email !== "" && !emailOk(data.email) ? "Enter a valid email address" : undefined}
            />
          </div>
        </Question>
      );
    ...
    case STEP.OPTION:
      return (
        <Question label="Would you like to rent or purchase?">
          <Choices
    ...
    case STEP.ATTENDANCE:
      return (
        <Question label="How many people will attend?">
          <TextField
            ref={firstFieldRef as React.MutableRefObject<HTMLInputElement | null>}
    ...
    case STEP.LOCATION:
      return (
        <Question label="Where is your event?">
          <TextField
            ref={firstFieldRef as React.MutableRefObject<HTMLInputElement | null>}
    ...
    case STEP.START:
      return (
        <Question label="When does it start?">
          <TextField
            ref={firstFieldRef as React.MutableRefObject<HTMLInputElement | null>}
            label="Start date"
            type="date"
    ...
    case STEP.END:
      return (
        <Question label="And when does it end?">
          <TextField
            ref={firstFieldRef as React.MutableRefObject<HTMLInputElement | null>}
            label="End date"
            type="date"
      ...
      return (
        <Question label="Anything else we should know?" optional>
          <textarea
            ...
            rows={4}
            placeholder="Tell us about your event… (Shift + Enter for a new line)"
    ...
    case STEP.POTABLE:
      return (
        <Question label="Is there potable water access on site?">
          <Choices
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold tracking-tight sm:text-3xl">
        {label}
        {optional && <span className="ml-2 text-base font-medium text-white/60">(optional)</span>}
      </h2>
...
function Choices({
  ...
  return (
    <div className="space-y-3" role="listbox" aria-label="Choose an option">
      {choices.map((c) => {
      ...
      <p className="pt-1 text-sm text-white/60">Tip: press the letter key to choose.</p>
    </div>
  );
}
...
function ThankYou({ onClose }: { onClose: () => void }) {
  return (
    <div className="text-center" aria-live="polite">
      ...
      <h1 className="mt-6 text-4xl font-extrabold tracking-tight sm:text-5xl">Thank you!</h1>
      <p className="mx-auto mt-4 max-w-md text-lg text-white/85">
        We&rsquo;ve received your request and will be in touch shortly. We can&rsquo;t wait to help
        make your event plastic-free.
      </p>
      <button
        type="button"
        onClick={onClose}
...
function ErrorState({ onRetry, onBack }: { onRetry: () => void; onBack: () => void }) {
  return (
    <div className="text-center" aria-live="assertive">
      ...
      <h1 className="mt-6 text-4xl font-extrabold tracking-tight sm:text-5xl">Something went wrong</h1>
      <p className="mx-auto mt-4 max-w-md text-lg text-white/85">
        We couldn&rsquo;t submit your request. Please check your connection and try again.
      </p>
